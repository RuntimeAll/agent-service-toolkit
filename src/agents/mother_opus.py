...
只输出一个 JSON（不要解释、不要 markdown fence），结构：
{
  ...
  "dna": {
    ...
    return f"""你是浙教版初中数学**解题专家** + 题库打标师。看这张母题图，**先像严谨的解题者那样真正把题解出来**（一步步算到最终答案，不许抄图、不许跳步），**再据你的解答**做 10 维 DNA 打标。母题是所有变式的基准，解错则全部变式跟着错——务必稳准。
🔴 **解题纪律**：题意/图形拿不准时按最自然的一种解释取定并解到底（别中途乱换解释）；推出干净答案后即定稿，**信任自己的正确推导，绝不为凑某个心里预设的"标准答案/期望值"而推翻正确结论、来回改答案、反复自我怀疑兜圈子**；与直觉不符最多复核一遍，确认无误就以你的推导为准、不再动摇。
