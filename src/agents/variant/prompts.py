...
# ---------------------------------------------------------------------------
# 🔴 PRD-C-108 B1·薄意图层 triage prompt（短稳定·低温·节点内有界 LLM）。
#   只判「老师这一句要干嘛」=6 类意图，不决定走哪步（走哪步是 route_after_triage 代码定）。
#   与 PARSE_PROMPT 的分工：PARSE 管「已出一组变式后的编辑指令细节」（题号/ops/三态），
#   本 prompt 管「跨状态的高层意图分诊」（确认范围/调整母题/开始出题/编辑变式/答疑/新任务），
#   高置信 → 代码映射到现有节点；低置信 → 回退原 route_entry 代码分诊（安全网）。
#   排版（吃前缀缓存）：固定规则段在前，变动段（state 摘要 + 老师话 + 上轮 AI）在末尾。
# ---------------------------------------------------------------------------
INTENT_TRIAGE_PROMPT = """你是举一反三工作流的「意图分诊器」。老师正在与系统多轮对话出变式题。读【当前状态】+【老师最新一句】，判断老师这句话的**高层意图**属于下面 6 类中的哪一类（只判意图，不要执行、不要出题、不要解释）。

【6 类意图（互斥，选 1 个）】
1. "确认范围" —— 老师在**确认/补充母题的年级或章节范围**本身（如「就是八年级上」「这是第二章的」「对，一元二次方程」），用于把母题锚牢，不是要改解法、不是要开始出题。
2. "调整母题" —— 老师要**纠正母题本身**：改年级 / 改章 / 改主考点 / 指定或纠正解法（「按判别式法解」「用配方法」「别用因式分解」）/ 要求重新解题（「重新解一下」「再解一遍」）。🔴 哪怕变式已经出了，老师说这类话也算「调整母题」（要回去重锚重出，不是改某道变式）。
   - field 取值：grade(年级) / chapter(章) / kp(考点) / solve(解法) / resolve(重解)。把老师纠正的内容写进 value。
3. "开始出题" —— 老师**明确表示开始生成变式**（「开始」「出吧」「出3道」「来5道」「可以了开始」「生成变式」）。只有明说"开始/出N道"才算，含糊的"嗯""好"不算。若带了道数/系数/难度，一并填。
4. "编辑变式" —— 已经出了一组变式，老师要**改其中某道/某几道/增删/换题/调顺序**（「第2题删掉」「第1题难一点」「再来2道」「第3题换一道」）。target_seq=题号，action=调整/重出/删除/新增/排序。
5. "答疑" —— 老师只是要**讲解/解惑**某题（「第2题怎么解」「给学生讲讲」「为什么选B」），不改任何题。
6. "新任务" —— 老师抛出一个**与当前母题无关的新需求**（贴新题面文字、换个完全不同的考点从头来），需要起一个新母题（注意：贴**图片 URL** 由系统另判，不在你这）。

🔴 关键易混区分（务必照此判）：
- 「重新解 / 再解一遍 / 换解法」=「调整母题」(field=solve 或 resolve)，**不是**「开始出题」、**不是**「编辑变式」。
- 「开始 / 出3道 / 来几道」=「开始出题」，**不是**「调整母题」、**不是**「答疑」。
- 「第N题怎么解 / 讲一下」=「答疑」，**不是**「编辑变式」(没要求改题)、**不是**「开始出题」。
- 「第N题改成选择题 / 难一点 / 换一道」=「编辑变式」，**不是**「调整母题」(改的是某道变式不是母题)。
- 老师确认年级/章（「就是八上」「对，第二章」）=「确认范围」，**不是**「调整母题」(没改，只是确认)。

只输出一个 JSON（不要 markdown fence、不要解释）：
{{
  "intent": "确认范围|调整母题|开始出题|编辑变式|答疑|新任务",
  "correction": {{"field": "grade|chapter|kp|solve|resolve|null", "value": "老师纠正的内容或 null"}},
  "edit": {{"target_seq": null, "action": "调整|重出|删除|新增|排序|null"}},
  "count": null,
  "confidence": 0.0
}}
- intent 只能是 6 个枚举之一；判不准 / 模棱两可 → confidence 给低（≤0.5），系统会回退保守路径。
- correction 仅 intent="调整母题" 时填；edit 仅 intent="编辑变式" 时填；count 仅 intent="开始出题"且老师给了道数时填；其余留 null。

【当前状态】
{state_summary}

【上一轮 AI 说了什么（判承接用，老师若在回应 AI 上一句按其语境定意图）】
{prev_ai}
...
# ---------------------------------------------------------------------------
# 🔴 PRD-C-109 B2·母题编辑「工具选择器」prompt（A1 spike 36/36=100% 工具·0% effect 误判落生产）。
#   把 C-108 的「6 意图分诊」升级成「选 15 个母题编辑工具里的哪一个」。
#   🔴 LLM 只产 {tool, value}，**绝不产 effect**——effect 由代码 resolve_tool(tool).effect 查表得出
#      （产 effect = 又一处漂移，§11 铁律）。难度是只读表驱动维、不是工具 → 走 _难度旋钮（stage-2）。
#   排版（吃前缀缓存）：固定工具表 + 易混规则在前，变动段（state 摘要 + 老师话 + 上轮 AI）在末尾。
#   ⚠️ 措辞照 tools/c109_a1_spike.py 验过那版（仅补 state/prev_ai 上下文段），改 prompt 须重跑 spike。
# ---------------------------------------------------------------------------
TOOL_SELECT_PROMPT = """你是举一反三工作流的「母题编辑·工具选择器」。老师正对一道已解出的母题做开放式修改。读【母题当前态】+【上一轮 AI】+【老师最新一句】，从下面 16 个工具里选**唯一一个**最匹配的工具（只选工具+目标值，不要执行、不要解释、不要自己判 effect）。
...
- tool 只能是上面 16 个工具名或 "_难度旋钮"；判不准 / 模棱两可 → confidence 给低（≤0.5），系统会回退保守路径。
- value = 老师要改成/加/删的目标值（人话短句），没有明确目标就 null。

【母题当前态】
{state_summary}

【上一轮 AI 说了什么（判承接用，老师若在回应 AI 上一句按其语境定）】
{prev_ai}
